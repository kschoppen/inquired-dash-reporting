...
function renderWeekly(d) {
  ...
  const movedBody = (moved && moved.length
      ? `<div class="tscroll"><table class="bd"><thead><tr><th>Deal</th><th>Pipeline</th><th>From → to</th><th>Amount</th><th>Company size</th></tr></thead><tbody>${moved.map((x) => `<tr><td><a class="lnk" href="${x.url}" target="_blank" rel="noopener">${escapeHtml(x.name)} ↗</a></td><td>${x.pipeline || "—"}</td><td>${x.from || (/^closed/i.test(x.to || "") ? "—" : "New")} → ${x.to || "—"}</td><td>${fmt$(x.amount)}</td><td>${x.segment || "—"}</td></tr>`).join("")}</tbody></table></div>`
  ...
  const movedDz = rdDz("Deals that moved this week", moved ? `<b>${moved.length}</b> deals changed stage` : "deals that changed stage this week", movedBody, { cls: "inner" });
